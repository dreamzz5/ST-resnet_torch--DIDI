...
def build_graph(data, grid_size, type):
    ...
    for i in range(all_fea.shape[0]):
        for j in range(all_fea.shape[0]):
            ...
            w=np.corrcoef(np.vstack((all_fea[i][:-1], all_fea[j][1:])))[0,1]
            graph[i][j] = w
    graph=np.nan_to_num(graph)
    graph=graph.reshape(19,18,19,18)
    if type=='in':
        np.save('./guide_BP_result/in_corrcoef',graph)
    if type=='out':
        np.save('./guide_BP_result/out_corrcoef',graph)
